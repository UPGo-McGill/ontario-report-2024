...
# Ottawa rent
(rent_2021 <- get_census("CA21", regions = list(CSD = "3506008"), 
...
scraped_type |> 
  filter(city_type %in% city_vec) |> 
  filter(month %in% c(yearmonth("2022-07"),
                      yearmonth("2023-12"))) |> 
...
fig_OT_1 <- 
  ...
  geom_sf(data = filter(CSD, city == "Ottawa"), fill = "grey",
          colour = "transparent") +
  ...
  coord_sf(xlim = st_bbox(filter(CSD, city == "Ottawa"))[c("xmin", "xmax")],
           ylim = st_bbox(filter(CSD, city == "Ottawa"))[c("ymin", "ymax")]) +
  theme_void() +
  theme(plot.margin = unit(c(0, 10, 10, 0), "pt"), 
        legend.position = "bottom", legend.key.width = unit(30, "pt"), 
        text = element_text(family = "Futura"))

ggsave("output/figure_OT_1.png", fig_OT_1, width = 4, height = 4, units = "in")
...
ggsave("output/figure_OT_2.png", fig_OT_2, width = 4, height = 4)
...
ggsave("output/figure_OT_3.png", fig_OT_3, width = 4, height = 4)
...
ggsave("output/figure_OT_4.png", fig_OT_4, width = 4, height = 4)
...
rent_cause <- 
  ...
  bind_rows() |> 
  semi_join(filter(reg, name_CSD == "Ottawa"))
...
city_rent_table |> 
  ...
  filter(name_CSD %in% c("Ontario", "Ottawa")) |> 
...
# Principal-residence restrictions are working in Ottawa ------------------
