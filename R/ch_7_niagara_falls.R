#### Chapter 6: Niagara Falls fact sheet ############################################
...
# Niagara Falls rent
(rent_2021 <- get_census("CA21", regions = list(CSD = "3526043"), 
...
fig_NF_1 <- 
  ...
  geom_sf(data = filter(CSD, city == "Niagara Falls"), fill = "grey",
          colour = "transparent") +
  ...
  scale_fill_stepsn(name = "STRs / dwelling", 
                    ...
                    breaks = c(0, 0.006, 0.012, 0.018, 0.024, 0.03), 
                    limits = c(0, 0.03), oob = scales::squish,
                    labels = scales::percent) +
  coord_sf(xlim = st_bbox(filter(CSD, city == "Niagara Falls"))[
    c("xmin", "xmax")],
           ylim = st_bbox(filter(CSD, city == "Niagara Falls"))[
             c("ymin", "ymax")]) +
  theme_void() +
...
ggsave("output/figure_NF_1.png", fig_NF_1, width = 4, height = 4, units = "in")
...
ggsave("output/figure_NF_2.png", fig_NF_2, width = 4, height = 4)
...
ggsave("output/figure_NF_3.png", fig_NF_3, width = 4, height = 4)


# The impact of STR activity on residential rents in Niagara Falls --------
...
rent_cause <- 
  ...
  bind_rows() |> 
  semi_join(filter(reg, name_CSD == "Niagara Falls"))
...
city_rent_table |> 
  filter(name_CSD == "Niagara Falls")


# Better STR regulations would lower rents in Niagara Falls --------------------
